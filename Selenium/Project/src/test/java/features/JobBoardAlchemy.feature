Feature: JobBoard Alchemy website activities
  ...
  Scenario Outline: Activity 9 - Create a job listing using the backend
    ...
    And User filled up the title "<JobTitle>" of the job listing
    And we scroll down through the web page to fill job data
    Then we filled up the job listing with data "<Location>" , "<CompanyName>", "<CompanyWebsite>"
    And User clicks on the publish button followed by View Job
    Then User verifies that JobListing with title "<JobTitle>" is posted successfully


    Examples:
      | Username | Password | websiteTitle                            | JobTitle           | Location  | CompanyName | CompanyWebsite          |
      | root     | pa$$w0rd | Dashboard ‹ Alchemy Jobs — WordPress    | Java Developer III | Hyderabad | CK Labs     |  https://www.cklabs.org |